...
def initialize(context):
    ...
    """
    # 导入生成的因子数据
    g.stock = dataGet.get_tushare_daily(context.data_dir, context.security, start_date=context.config['data_start_date'],
                                        end_date=context.config['data_end_date'])
    ...
    trade_cal = g.stock.index
    trade_cal = trade_cal.date
    # 根据trade_cal，将context相应的属性初始化
    context.init_trade_cal(trade_cal)

    key = list(context.config['factor_control'].keys())[0]
    context.factor_name = key

    # 生成相应的因子
    module_name, func_name = context.config['factor_control'][key]['func'].split('.')
    module = importlib.import_module(module_name)
    factor_position_function = getattr(module, func_name)
    params = context.config['factor_control'][key]

    del params['func']
    g.position = factor_position_function(g.stock, **params)
    # g.position = factor_position_function(g.stock)
...
def handle_data(context):
    ...
    if not (context.security in context.portfolios[3].positions):
        ...
        # 根据以上指标构建一个多空逻辑，即
    current_side = g.position['position'][context.current_dt]
    current_side_shift = g.position['position'].shift()[context.current_dt]
    if not pd.isnull(current_side) and not pd.isnull(current_side_shift):
        if (current_side > current_side_shift):
            # 如果股票不在子账户1的持仓中，全仓买入
...
def multi_run(CONF={}):
    ...
    configs = local_config.generate_config_combinations(CONF_merge)[0:3]
